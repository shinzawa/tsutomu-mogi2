...
class AttendanceIndexTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        Carbon::setTestNow(Carbon::create(2026, 02, 15));
    ...
    /** @test */
    public function 初期表示で現在の月が表示される()
    {
        $response = $this->actingAs($this->user)
            ->get(route('attendance.index'));
    ...
    public function 前月ボタン押下で前月の勤怠一覧が表示される()
    {
        ...
        $response = $this->actingAs($this->user)
            ->get(route('attendance.index', ['year' => $year, 'month' => $month]));
        ...
        $this->user->attendances()
            ...
            ->get()
            ->each(function ($attendance) use ($response) {
                $response->assertDontSee($attendance->work_date->format('m/d'));
            });
    }
    ...
    public function 詳細ボタン押下でその日の勤怠詳細画面に遷移する()
    {
        ...
        $response->assertViewIs('attendance.show'); // ビュー名はプロジェクトに合わせて変更
